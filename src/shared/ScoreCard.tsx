...
export function ScoreCard({ performance, isLoading, userName, onRefresh }: ScoreCardProps) {
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-xl">Performans Durumu</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  const { score, color, breakdown, recommendations } = performance;
  const colorClasses = PerformanceService.getColorClasses(color);
        ...
        <div className="flex justify-between items-start">
          <div>
            <CardTitle className={`text-xl ${colorClasses.text}`}>
              {getColorEmoji()} Performans Durumu
            </CardTitle>
      ...
      <CardContent>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-white rounded-lg p-3 border">
            ...
            <p className="text-xs text-gray-600">Birebir Görüşmeler</p>
          </div>
          ...
          <div className="bg-white rounded-lg p-3 border">
            ...
            <p className="text-xs text-gray-600">Yoklama</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg p-3 border mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <span className="text-sm">Eğitim Saati</span>
            </div>
            <span className="text-sm font-medium">{('education' in breakdown ? breakdown.education : 0)}%</span>
          </div>
        </div>
